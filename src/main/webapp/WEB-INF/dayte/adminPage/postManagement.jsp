...
<div class="h-100 px-3" style="padding-top: 70px; min-width:375px;">
    <div class="px-5">
        ...
        <div class="mb-2 d-flex flex-row align-items-center justify-content-between">
            ...
            <div>
                <input type="button" id="delChkPostBtn" class="btn btn-dark px-3 py-2 m-0 me-2 adminUserFS"
                       value="선택 삭제">
            </div>
        </div>
                        ...
                        <c:forEach var="post" items="${postList.content}">
                            <tr style="height: 50px;">
                                <td class="text-center align-middle">
                                    <input class="form-check-input chkGrp" name="chkPost" type="checkbox"
                                ...
                                <td class="text-center align-middle h4 fw-normal">
                                    <a class="d-block  h-100 w-100 mb-0 text-decoration-none text-dark h4 fw-normal"
                                       href="/post/${post.id}">${post.title} [${post.replyList.size()}]</a>
                                </td>
                                <td class="text-center align-middle h4 fw-normal">${post.user.nickName}</td>
                                <td class="text-center align-middle h4 fw-normal"><fmt:formatDate
                                        value="${post.createDate}"
                                        pattern="yyyy.MM.dd HH:mm:ss"/></td>
                                <td class="text-center align-middle">
                                    <a class="btn btn-dark px-0 ms-1 d-block adminUserFS" id="post-delete">삭제</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </form>
                ...
                <nav aria-label="Page navigation ">
                    <ul class="pagination justify-content-center">
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li class="page-item"><a class="page-link text-dark adminUserFS"
                                                         href="?page=0&field=${param.field}&word=${param.word}">처음</a>
                                </li>
                                <li class="page-item"><a class="page-link text-dark adminUserFS"
                                                         href="?page=${postList.number-1}&field=${param.field}&word=${param.word}">&larr;</a>
                                </li>
                            </c:otherwise>
                        </c:choose>
                        ...
                        <c:forEach begin="${postStartPage}" end="${postEndPage}" var="i">
                            <c:choose>
                                <c:when test="${postList.pageable.pageNumber+1 == i}">
                                    <li class="page-item disabled"><a class="page-link px-3 text-dark adminUserFS"
                                                                      href="?page=${i-1}&field=${param.field}&word=${param.word}">${i}</a>
                                    </li>
                                </c:when>
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link px-3 text-dark adminUserFS"
                                                             href="?page=${i-1}&field=${param.field}&word=${param.word}">${i}</a>
                                    </li>
                                </c:otherwise>
                            </c:choose>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li class="page-item "><a class="page-link text-dark adminUserFS"
                                                          href="?page=${postList.number+1}&field=${param.field}&word=${param.word}">&rarr;</a>
                                </li>
                                <li class="page-item "><a class="page-link text-dark adminUserFS"
                                                          href="?page=${postList.totalPages-1}&field=${param.field}&word=${param.word}">마지막</a>
                                </li>
                            </c:otherwise>
                        </c:choose>
                    </ul>
...
<script>
    ...
    $("#post-delete").on("click", function () {
        let id = $(this).closest("tr").find(".postId").text();
        if (confirm(id + "번 포스트를 삭제 하시겠습니까?")) {
            fetch("/post/" + id, {
                method : "DELETE",
                headers: {
