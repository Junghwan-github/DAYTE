        ...
        <c:if test="${!empty noticeList}">
            <table id="noticeTable">
                ...
                <c:forEach var="notice" items="${noticeList.content}">
                    <tr class="noticeTrStyle <c:if test='${notice.priority > 0}'>priorityNotZero</c:if>" >
                        <td class="tdStyle">${notice.no}</td>
                        <td class="tdStyle"><a href="/notice/${notice.no}" class="tdTitle">${notice.title}</a></td>
                ...
                <ul>
                    ...
                    <c:forEach var="i" begin="${startPage}" end="${endPage}">
                        <li class="paginationNum">
                            <a class="page-link <c:if  test='${currentPage == i}'>active</c:if>" href="?page=${i}">${i + 1}</a>
                        </li>
                    </c:forEach>
